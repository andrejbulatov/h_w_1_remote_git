...
# 
    ...
    def delete(self, number_in_list: str) -> str:
        for contact in self._phone_book:
            if number_in_list == contact.get('id'):
                delete_person = self._phone_book[int(number_in_list) - 1]['name']
                self._phone_book.pop(int(number_in_list) - 1)
        return delete_person


    def change_order_index(self):
        index = 0
        for contact in self._phone_book:
            contact['id'] = str(index + 1)
            index += 1
